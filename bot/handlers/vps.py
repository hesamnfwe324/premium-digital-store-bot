...
def _plan_specs_text(plan: dict, lang: str) -> str:
    ...
    return (
        ...
        f"💡 Best for: <i>{use_case}</i>"
    )


# ─── Menu entry (wide button in main menu) ────────────────────────────────────
...
async def handle_vps_type(callback: CallbackQuery, user_lang: str = "en"):
    ...
    try:
        await callback.message.edit_text(text, reply_markup=get_vps_location_keyboard(server_type, user_lang), parse_mode="HTML")
    except Exception:
        await callback.message.answer(text, reply_markup=get_vps_location_keyboard(server_type, user_lang), parse_mode="HTML")
    await callback.answer()
...
async def handle_vps_location(callback: CallbackQuery, user_lang: str = "en"):
    ...
    try:
        await callback.message.edit_text(text, reply_markup=get_vps_plan_keyboard(server_type, location_id, user_lang), parse_mode="HTML")
    except Exception:
        await callback.message.answer(text, reply_markup=get_vps_plan_keyboard(server_type, location_id, user_lang), parse_mode="HTML")
    await callback.answer()


# ─── Plan → GPU or Duration ───────────────────────────────────────────────────
...
async def handle_vps_gpu(callback: CallbackQuery, user_lang: str = "en"):
    ...
    try:
        await callback.message.edit_text(text, reply_markup=get_vps_duration_keyboard(server_type, location_id, plan_id, gpu_id, user_lang), parse_mode="HTML")
    except Exception:
        await callback.message.answer(text, reply_markup=get_vps_duration_keyboard(server_type, location_id, plan_id, gpu_id, user_lang), parse_mode="HTML")
    await callback.answer()
...
async def handle_vps_duration(callback: CallbackQuery, user_lang: str = "en"):
    ...
    text = (
        ...
        + f"\n\n<b>{plan.get('name', '')}</b>"
        + f"\n📅 {dur_name}  —  <b>${total:.2f} USDT total</b>"
    )
    try:
        await callback.message.edit_text(text, reply_markup=get_vps_os_keyboard(server_type, location_id, plan_id, gpu_id, dur_id, user_lang), parse_mode="HTML")
    except Exception:
        await callback.message.answer(text, reply_markup=get_vps_os_keyboard(server_type, location_id, plan_id, gpu_id, dur_id, user_lang), parse_mode="HTML")
    await callback.answer()
...
async def handle_vps_os(callback: CallbackQuery, user_lang: str = "en"):
    ...
    parts = callback.data.split(":")
    server_type, location_id, plan_id, gpu_id, dur_id, os_id = parts[2], parts[3], parts[4], parts[5], parts[6], parts[7]
...
# ─── Confirm → Crypto payment selection ───────────────────────────────────────
...
async def handle_vps_confirm(callback: CallbackQuery, state: FSMContext, user_lang: str = "en"):
    ...
    # Store full order in FSM state for payment step
    ...
    try:
        await callback.message.edit_text(text, reply_markup=get_vps_crypto_keyboard(user_lang), parse_mode="HTML")
    except Exception:
        await callback.message.answer(text, reply_markup=get_vps_crypto_keyboard(user_lang), parse_mode="HTML")
    await callback.answer()


# ─── Crypto selected → Show address + QR ─────────────────────────────────────
...
async def handle_vps_pay_crypto(callback: CallbackQuery, state: FSMContext, user_lang: str = "en"):
    ...
    if not address:
        await callback.answer("❌ This currency is temporarily unavailable.", show_alert=True)
        return

    # Store chosen currency in FSM
...
# ─── TXID received → Create payment, notify admins ───────────────────────────
...
async def handle_vps_txid(message: Message, state: FSMContext, session: AsyncSession, user_lang: str = "en"):
    ...
    # Create a Payment record in DB (without an Order, VPS-specific)
    ...
    try:
        ...
        logger.error(f"VPS payment DB error: {e}")
        payment_id = None
    ...
    admin_msg = (
        f"🖥 <b>New VPS Order — Payment Received</b>\n\n"
        ...
        f"<b>💳 Payment:</b>\n"
        f"{emoji} Currency: <b>{net_name}</b>\n"
        f"💰 Amount: <b>${total:.2f} USDT</b>\n"
        f"🔑 TXID/Link: <code>{txid}</code>\n"
        f"🧾 Ref: <code>{payment_ref}</code>"
    )

    kb = get_vps_payment_admin_keyboard(payment_id) if payment_id else None

    for admin_id in settings.ADMIN_IDS:
        try:
            await message.bot.send_message(admin_id, admin_msg, parse_mode="HTML", reply_markup=kb)
...
async def handle_admin_vps_confirm(callback: CallbackQuery, session: AsyncSession):
    ...
    try:
        await callback.bot.send_message(
            ...
            f"💰 <b>${payment.amount_usdt:.2f} USDT</b> received.\n\n"
            "🖥 Your VPS/Server is being set up now.\n"
            "⏱ Setup time: <b>1–4 hours</b>\n\n"
            "Our team will message you with access credentials as soon as the server is ready.\n"
            "For any questions, contact support via /start → Support.",
            parse_mode="HTML",
        )
    except Exception as exc:
    ...
    await callback.answer("✅ Payment confirmed & customer notified!", show_alert=True)


# ─── Admin: Reject VPS payment ────────────────────────────────────────────────
...
async def handle_admin_vps_reject(callback: CallbackQuery, session: AsyncSession):
    ...
    try:
        await callback.bot.send_message(
            ...
            "We could not verify your VPS payment. Please check the transaction and try again, "
    ...
    await callback.message.edit_reply_markup(reply_markup=None)
    await callback.answer("❌ Payment rejected & customer notified.", show_alert=True)
