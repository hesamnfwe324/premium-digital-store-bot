...
class Product(Base):
    ...
    deal_ends_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    ...
    stock_notifications: Mapped[List["StockNotification"]] = relationship("StockNotification", back_populates="product")

    @property
    def is_on_deal(self) -> bool:
        if self.original_price is None or self.original_price <= self.price_usdt:
            return False
        if self.deal_ends_at is None:
            return True
        from datetime import timezone
        now = datetime.now(timezone.utc)
        deadline = self.deal_ends_at
    ...
    def discount_percent(self) -> int:
        if not self.original_price or self.original_price <= 0:
            return 0
        return round((1 - (self.price_usdt / self.original_price)) * 100)
